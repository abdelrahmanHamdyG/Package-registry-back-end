...
describe('Testing Routes', () => {
  ...
  it('should return 200 for the default invalid endpoint', async () => {
    const response = await request(app).get('/');
    expect(response.status).toBe(200);
    expect(response.text).toBe("You are not at a valid endpoint. Try using '/package' or other routes.");
  });
